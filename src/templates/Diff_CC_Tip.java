...
*/		
		...
		int rowCount = sheet1.getLastRowNum();
			...
			String ccTip = driver.findElement(By.xpath("//div[contains(.,'Credit Card Tip')]/span")).getText().replaceAll("[a-zA-Z $ ₹ , :]", "");
			double ccTipAmt = Double.parseDouble(ccTip);
			
			sheet1.getRow(1).createCell(4).setCellValue(ccTipAmt);
			...
			for(int i = 2; i < rowCount;i++)
			{
				...
				try{
					//Get the CC TIP Amount
					String ccTip1 = driver.findElement(By.xpath("//div[contains(.,'Credit Card Tip')]/span")).getText().replaceAll("[a-zA-Z $ ₹ , :]", "");
					double ccTipAmt1 = Double.parseDouble(ccTip1);
					
					sheet1.getRow(i).createCell(4).setCellValue(ccTipAmt1);
				}
				catch(Exception e)
				{
	    ...
	    driver.findElement(By.xpath("//form[@name='payRoll']/div[3]/div/div/div/div/input")).sendKeys("This week");
	    //Press the Enter key
...
*/		
		...
		else
		{
			...
			//Get the CC TIP Amount
			String ccTippay = driver.findElement(By.xpath("//table[@class='table table-bordered ng-table-responsive table-condensed ng-scope ng-table']/tbody/tr["+rows.size()+"]/td[11]")).getText().replaceAll("[a-zA-Z $ ₹ , :]", "");
			double ccTipAmtpay = Double.parseDouble(ccTippay);
			
			sheet1.getRow(1).createCell(5).setCellValue(ccTipAmtpay);
			...
			for(int i = 2; i < rowCount;i++)
			{
			    ...
			    driver.findElement(By.xpath("//form[@name='payRoll']/div[1]/div/div/a")).click();
			    //Enter the required Employee
			    driver.findElement(By.xpath("//form[@name='payRoll']/div[1]/div/div/div/div/input")).sendKeys(sheet1.getRow(i).getCell(3).getStringCellValue());
			    //Press the Enter key
			    driver.findElement(By.xpath("//form[@name='payRoll']/div[1]/div/div/div/div/input")).sendKeys(Keys.ENTER);
				...
				else
				{
					...
					//Get the CC TIP Amount
					String ccTip1pay = driver.findElement(By.xpath("//table[@class='table table-bordered ng-table-responsive table-condensed ng-scope ng-table']/tbody/tr["+rows.size()+"]/td[11]")).getText().replaceAll("[a-zA-Z $ ₹ , :]", "");
					double ccTipAmt1pay = Double.parseDouble(ccTip1pay);
					
					sheet1.getRow(i).createCell(5).setCellValue(ccTipAmt1pay);
				}
			}
		}
		...
		for(int  i = 1; i <rowCount;i++)
		{
			if(sheet1.getRow(i).getCell(4).getNumericCellValue() == sheet1.getRow(i).getCell(5).getNumericCellValue())
			{
				sheet1.getRow(i).createCell(6).setCellValue(0);
			}
			else
			{
				double diff = sheet1.getRow(i).getCell(4).getNumericCellValue() - sheet1.getRow(i).getCell(5).getNumericCellValue();
				sheet1.getRow(i).createCell(6).setCellValue(diff);
			}
		}
